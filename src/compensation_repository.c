...
void create_compensation_table() {
  ...
  if (db == NULL) {
    ...
    char sql_raw[1024];
    sprintf(sql_raw,
            "CREATE TABLE IF NOT EXISTS %s (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, DURATION INT NOT NULL, "
...
void compensation_create_entry(compensation_entry_t compensation_entry) {
  ...
  if (db == NULL) {
    ...
    char sql_raw[1024];
    sprintf(sql_raw, "INSERT INTO %s (DURATION, CREATED) values (%d, %d)", COMPENSATION_TABLE_NAME,
            compensation_entry.duration, compensation_entry.created);
